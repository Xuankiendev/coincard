...
def follow(message, bot):
    args = message.text.split(maxsplit=1)
    if len(args) < 2:
        bot.reply_to(message, 'Vui lòng cung cấp username\nVí dụ: /follow Người Dùng Cần Buff')
        return
    username = args[1]
    if username[0] != '@':
        username = '@' + username
    ...
    if not token:
        ...
        return
    try:
        response = session.post('https://tikfollowers.com/api/free', headers={'accept': '*/*','accept-language': 'vi-VN,vi;q=0.9','content-type': 'text/plain;charset=UTF-8','origin': 'https://tikfollowers.com','priority': 'u=1, i','referer': 'https://tikfollowers.com/free-tiktok-followers','sec-ch-ua': '"Chromium";v="134", "Not:A-Brand";v="24", "Google Chrome";v="134"','sec-ch-ua-mobile': '?0','sec-ch-ua-platform': '"Windows"','sec-fetch-dest': 'empty','sec-fetch-mode': 'cors','sec-fetch-site': 'same-origin','user-agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/134.0.0.0 Safari/537.36'}, data=json.dumps({"type":"follow","q":username,"google_token":google_token,"token":token}))
        result = json.loads(response.text)
        bot.reply_to(message, 'Đang gửi yêu cầu')
        data = result['data']
    except:
        bot.reply_to(message, 'Yêu cầu thất bại')
        return
    send_data = {"google_token":google_token,"token":token,"data":data,"type":"follow"}
    try:
        ...
        if result["type"] == "success":
            bot.reply_to(message, 'Follow thành công')
            ...
            bot.reply_to(message, 'Lỗi server khi tăng follow')
    except:
        bot.reply_to(message, 'Lỗi mạng khi tăng follow')
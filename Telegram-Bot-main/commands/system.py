...
-----------------------------------------
• Thời gian lấy thông tin: {execution_time:.2f} giây
"""

def system(message, bot):
    wait_message = "Vui lòng chờ 1 chút trong khi bot kiểm tra thông tin hệ thống..."
    bot.reply_to(message, wait_message)
    try:
        sys_info = system_info()
        bot.reply_to(message, sys_info)
    except Exception as e:
        bot.reply_to(message, f"Lỗi khi lấy thông tin hệ thống: {str(e)}")
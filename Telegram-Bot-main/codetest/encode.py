...
def _rd():
    return "".join(__import__("random").sample(
        [chr(i) for i in range(97, 122)], k=10))
...
def rd():
    return "_" + "".join(__import__("random").sample([str(i) for i in range(1, 20)], k=4))
...
def obfstr(v):
    ...
    if v == "":
        ...
        _str_ = f"(lambda : (lambda : (lambda : (lambda : (lambda : (lambda : (lambda : (lambda : {_join}(( {_list}({_map}({_hexrun}, {x})) )))())())())())())())())()"
        return _str_
...
def _moreobf(tree):
    ...
    def junk(en, max_value):
        cases = []
        line = max_value + 1
        for i in range(random.randint(112, 224)):
            case_name = "__"+rd()
            case_body = [
                ast.If(
                    test=ast.Compare(
...
if len(sys.argv) != 3:
    print("Usage: python encode.py <filename> <mode>")
    sys.exit(1)
...
#
#     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
#            Phật độ, CODE không lỗi, tôi ưu không BUG
#     ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# Encode By Vũ Xuân Kiên From Team V X K Community Service 
# Liên Hệ Nếu Cần Encode File:
# Hotline: 0345864723
# Email: vxkiue@gmail.com, vxkitvn@gmail.com (Business Only)
# Developer By Team V X K Community Service
"""
anti = r"""
...
def vv():
    ...
    raise ValueError('>> DETECTED!!') from None
...
def bytecode():
    VUXUANKIEN = globals().update
    if True:
        VUXUANKIEN({ngoac}**{ngoac} _hex({_uni("en")}): {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("marshal")}))).items() if {_movdiv}({_temp}) and {_temp1} == _hex({_uni("loads")}){_ngoac}, **{ngoac}{_temp1}: {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("marshal")}))).items() if {_movdiv}({_temp}) and {_temp1} != _hex({_uni("loads")}){_ngoac}{_ngoac})
    else:"VUXUANKIEN"
    if 1>2:
        {obfint(3)}
    else:
        VUXUANKIEN({ngoac}**{ngoac}_hex({_uni("july")}): {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("zlib")}))).items() if {_movdiv}({_temp}) and {_temp1} == _hex({_uni("decompress")}){_ngoac}, **{ngoac}{_temp1}: {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("zlib")}))).items() if {_movdiv}({_temp}) and {_temp1} != _hex({_uni("decompress")}){_ngoac}{_ngoac})
    VUXUANKIEN({ngoac}**{ngoac}_hex({_uni("birth")}): {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("bz2")}))).items() if {_movdiv}({_temp}) and {_temp1} == _hex({_uni("decompress")}){_ngoac}, **{ngoac}{_temp1}: {_temp} {_f} {_temp1}, {_temp} {_i} {_vars}({___import__}(_hex({_uni("bz2")}))).items() if {_movdiv}({_temp}) and {_temp1} != _hex({_uni("decompress")}){_ngoac}{_ngoac})
    VUXUANKIEN()
    VUXUANKIEN({ngoac}**{ngoac}_hex({_uni("_")}): {_t} {_f} {_temp1}, {_t} {_i} {_vars}({___import__}(_hex({_uni("base64")}))).items() if {_movdiv}({_t}) and {_temp1} == _hex({_uni("b85decode")}){_ngoac}, **{ngoac}{_temp1}: {_t} {_f} {_temp1}, {_t} {_i} {_vars}({___import__}(_hex({_uni("base64")}))).items() if {_movdiv}({_t}) and {_temp1} != _hex({_uni("b85decode")}){_ngoac}{_ngoac})
    VUXUANKIEN()
    VUXUANKIEN({ngoac}**{ngoac}_hex({_uni("VUXUANKIEN")}): {_t} {_f} {_temp1}, {_t} {_i} {_vars}({___import__}(_hex({_uni("builtins")}))).items() if {_movdiv}({_t}) and {_temp1} == _hex({_uni("exec")}){_ngoac}, **{ngoac}{_temp1}: {_t} {_f} {_temp1}, {_t} {_i} {_vars}({___import__}(_hex({_uni("builtins")}))).items() if {_movdiv}({_t}) and {_temp1} != _hex({_uni("eval")}){_ngoac}{_ngoac})
bytecode()

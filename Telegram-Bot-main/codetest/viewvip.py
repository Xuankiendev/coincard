    ...
    try:
        for _ in range(2):
            if not send_buff_once(link, quantity, service_id):
                ...
                return
            time.sleep(600)
        bot.send_message(chat_id, f"Đa buff lai view cho {link}.")
    except Exception as e:
        bot.send_message(chat_id, f"Co loi xay ra: {e}")

def viewvip(message, bot):
    ...
    try:
        ...
        if len(args) < 3:
            bot.send_message(message.chat.id, "Sai cu phap: /viewvip <link> <so luong>")
            return
        link = args[1]
        quantity = int(args[2])
        ...
        if quantity <= 0:
            ...
            return
        threading.Thread(target=send_buff_repeat, args=(link, quantity, SERVICE_ID_VIEW, message.chat.id), daemon=True).start()
        bot.send_message(message.chat.id, f"Đa gui buff view VIP cho {link} x{quantity}, se tu buff lai sau 10 phut.")
    except Exception as e:
